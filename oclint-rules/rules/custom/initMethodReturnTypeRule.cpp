...
class initMethodReturnTypeRule : public AbstractASTVisitorRule<initMethodReturnTypeRule>
{
public:
    virtual const string name() const override
    {
        return "init method return type must be instancetype";
    }
...
.. code-block:: cpp
    ...
    */
    ...
    bool VisitObjCMethodDecl(ObjCMethodDecl *node)
    {
        ...
        if (0 == strncmp(methodName.c_str(), "init", strlen("init"))) {
	  if (node->getReturnType().getAsString() != "instancetype") {
	    addViolation(node, this, "init method return type must be instancetype.");
	  }
        }
        return true;
    }
...
static RuleSet rules(new initMethodReturnTypeRule());
